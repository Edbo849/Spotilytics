document.getElementById('importHistoryForm').addEventListener('submit', function (event) {
    ...
    function updateProgress() {
        ...
        if (completedFiles === totalFiles) {
            ...
            `;
            setTimeout(function () {
                var modal = bootstrap.Modal.getInstance(document.getElementById('importHistoryModal'));
                modal.hide();
    ...
    function uploadFile(file) {
        ...
        xhr.addEventListener('error', function () {
            document.getElementById('loadingMessage').innerHTML = `
                <p style="color: red;">Upload Failed. An error occurred while uploading the files.</p>
                <div class="progress">
                    <div id="progressBar" class="progress-bar bg-danger" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
            `;
...
document.getElementById('deleteHistoryForm').addEventListener('submit', function (event) {
    event.preventDefault();
    ...
    xhr.addEventListener('load', function () {
        if (xhr.status === 200) {
            ...
            alert('Failed to delete listening history. Error: ' + xhr.responseText);
        }
    });
