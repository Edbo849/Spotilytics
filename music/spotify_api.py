...
def get_access_token(spotify_user_id: str) -> str:
    """
    Retrieve a valid access token for the given Spotify user ID.
    """
    tokens = SpotifyToken.objects.filter(spotify_user_id=spotify_user_id)
    if tokens.exists():
        token = tokens.first()
        if token.expires_in <= timezone.now():
            refresh_spotify_token(spotify_user_id, token.scope)
            token = SpotifyToken.objects.get(spotify_user_id=spotify_user_id)
        return token.access_token
    else:
        raise ValueError("No Spotify token found for the given Spotify user ID.")
...
def make_spotify_request(
    ...
    """
    Make a request to the Spotify API using the user's access token.
    """
    access_token = get_access_token(spotify_user_id)
    print(access_token)
...
def get_top_tracks(
    ...
    response = make_spotify_request(endpoint, spotify_user_id, params)
...
def get_top_artists(
    ...
    """
    params = {"limit": num, "time_range": time_range}
    return make_spotify_request("me/top/artists", spotify_user_id, params).get(
        "items", []
    )
...
def get_top_genres(
    ...
    """
    params = {"limit": num, "time_range": time_range}
    artists = make_spotify_request("me/top/artists", spotify_user_id, params).get(
        "items", []
    )
...
def get_recently_played(num: int, spotify_user_id: str) -> list[dict[str, Any]]:
    """
    Retrieve the user's recently played tracks.
    """
    params = {"limit": num}
    return make_spotify_request(
        "me/player/recently-played", spotify_user_id, params
    ).get("items", [])


def fetch_artist_albums(
    artist_id: str, spotify_user_id: str, single: bool
) -> list[dict[str, Any]]:
    """
    ...
    albums = make_spotify_request(
        f"artists/{artist_id}/albums", spotify_user_id, params
    ).get("items", [])
...
def fetch_artist_top_tracks(
    ...
    tracks = make_spotify_request(
        f"artists/{artist_id}/top-tracks", spotify_user_id, params
    ).get("tracks", [])
    return tracks[:num]


def get_track_details(track_id: str, spotify_user_id: str) -> dict[str, Any]:
    """
    Retrieve the details of a given track.
    """
    return make_spotify_request(f"tracks/{track_id}", spotify_user_id)


def get_artist(artist_id: str, spotify_user_id: str) -> dict[str, Any]:
    """
    Retrieve the details of a given artist.
    """
    return make_spotify_request(f"artists/{artist_id}", spotify_user_id)


def get_album(album_id: str, spotify_user_id: str) -> dict[str, Any]:
    """
    Retrieve the details of a given album.
    """
    return make_spotify_request(f"albums/{album_id}", spotify_user_id)


def get_similar_artists(artist_id: str, spotify_user_id: str) -> list[dict[str, Any]]:
    """
    Retrieve similar artists to a given artist.
    """
    return make_spotify_request(
        f"artists/{artist_id}/related-artists", spotify_user_id
    ).get("artists", [])


def get_similar_tracks(track_id: str, spotify_user_id: str) -> list[dict[str, Any]]:
    """
    Get similar tracks based on seed track.
    """
    params = {"seed_tracks": track_id, "limit": 10}
    data = make_spotify_request("recommendations", spotify_user_id, params)
    track_ids = [track["id"] for track in data["tracks"]]
    params_tracks = {"ids": ",".join(track_ids)}
    full_tracks_data = make_spotify_request("tracks", spotify_user_id, params_tracks)
    return full_tracks_data["tracks"]


def get_recently_played_full(spotify_user_id: str) -> list[dict[str, Any]]:
    """
